...
class TSocketServer<InProtocol: TProtocol, OutProtocol: TProtocol, Processor: TProcessor, Service> where Processor.Service == Service {
    ...
    public init(port: Int,
        ...
        if sock != nil {
            ...
            let cfaddr = address.withUnsafeBytes { unsafeRawBufferPointer -> CFData in
                let unsafeBufferPointer = unsafeRawBufferPointer.bindMemory(to: UInt8.self)
                return CFDataCreateWithBytesNoCopy(kCFAllocatorDefault, unsafeBufferPointer.baseAddress!, address.count, nil)
            }
